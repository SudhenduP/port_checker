...
port\_checker is a python based code using the following module to check
if ports are open on many servers.

Problem Statement
-----------------

In my day to day work, I always need the check whether port access to a
server is open on not. The usual way to do this is telnet. But many
times, the list of server-port could gets huge, and checking each
individually was taking a lot of effort. Hence this tool.
...
exe: port\_checker.exe

file: input/in\_server\_port.csv

#### Example of input for your in\_server\_port.csv file. Please don't change the header name
...
Please only use it if you want to change the script. The .exe is a
